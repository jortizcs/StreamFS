...
public class MySqlDriver {
    ...
    private static void setupMemcached(){
        //setup memcached
        String[] serverlist = { "133.11.168.9:11211"};
        ...
        //initialize memcached client w/compression
	...
	public String getRRType(String path){

        String rrTypeStr = (String)memcachedClient.get(DBQueryTypes.TYPE + path);
        if(rrTypeStr == null){
            Connection conn = null;
            ...
            try{
                ...
                String restResource2 = null;
                if(path.endsWith("/"))
                    restResource2 = path.substring(0, path.length()-1);
                else
                    restResource2 = path + "/";
                ps1.setString(2, restResource2);
        ...
        memcachedClient.set(DBQueryTypes.TYPE + path, rrTypeStr);
	...
	public JSONArray rrGetChildren(String path){
        String key = DBQueryTypes.GETC + path;
        String value = null;
        Connection conn = null;
        try{
            if((value=(String)memcachedClient.get(key)) == null){
                ...
                JSONArray children = (JSONArray)parser.parse(value);
                return children;
            }
        } catch (Exception e){
            logger.log(Level.WARNING, "", e);
	...
	public JSONArray rrGetAllPaths(){
        ...
        String allpathstr = (String)memcachedClient.get(DBQueryTypes.ALL_PATHS);
        Connection conn = null;
		try{
            if(allpathstr==null){
                conn = openConnLocal();
	...
	public JSONObject getContextGraph(String contextGraphId){
		...
		try {
			...
			while(rs.next()){
                ...
                os = (JSONObject) parser.parse(new String(mapBlob.getBytes(1L, (int)mapBlob.length())));
			}
		} catch (Exception e){
	...
	public JSONArray getPubIdsFromSnodes(JSONArray snodes){
		...
		try{			
			...
			for(int i=0; i<snodes.size(); i++){
				String thisPath = (String)snodes.get(i);
			...
			for (int i=0; i<nonStarred.size(); i++){
				...
				String thisPath = (String)nonStarred.get(i);
				String thisPath2 = null;
			...
			for(int i=0; i<starredList.size(); i++){
				...
				String thisPath = (String)starredList.get(i);
	...
	public void insertNewSubEntry(UUID subid, String alias, String uri, String destUrl, String destUri, UUID src_pubid, String wildcardPath){
		...
		try{
			...
			if(
				...
				(destUrl==null && destUri!=null && ((rsrcType=getRRType(destUri)) != null)) 
				&& (ResourceUtils.translateType(rsrcType)==ResourceUtils.MODEL_GENERIC_PUBLISHER_RSRC || ResourceUtils.translateType(rsrcType) == ResourceUtils.PROCESS_PUBLISHER_RSRC)){
	...
	public void removeSubEntries(UUID subid, JSONArray streams){
		try{
			for(int i=0; i<streams.size(); i++){
				UUID thisPubId = null;
				try{ thisPubId = UUID.fromString((String)streams.get(i)); } 
		...
		try{
			...
			if(numPubs>0){
				...
				for(int i=1; i<=numPubs; i++)
					ps.setString(i, (String)srcPubIds.get(i-1));
				ResultSet rs = ps.executeQuery();
				while(rs.next())
	...
	public JSONArray resolveStarredUri(String uri, String type){
		...
		try {
			if(uri.contains("*")){
				...
				uris.clear();
				Iterator keys = new HashSet(ht.keySet()).iterator();
    ...
    /**
     * Fetches a list of all symbolic links and the path they point to.
     */
    public JSONArray getAllSymlinkAndTargetPaths(){
        JSONArray linkAndTargets = new JSONArray();
		Connection conn = openConn();
		try{
			String query = "SELECT `symlink_uri`, `links_to` FROM `symlinks`";
			PreparedStatement ps = conn.prepareStatement(query);
			ResultSet rs = ps.executeQuery();
			while(rs.next()){
                JSONArray thispair = new JSONArray();
                thispair.add(rs.getString("symlink_uri"));
                thispair.add(rs.getString("links_to"));
				linkAndTargets.add(thispair);
            }
	...
	public void updateLastPropsTs(String uri, long timestamp){
        String key = DBQueryTypes.PROPS_TS + uri;
        String value = null;
		Connection conn = null;
		try{
