                    ...
                    worker.send({"type":"testprocedure"}, 
                        function(error, message){
                            ...
                            if(typeof error != 'undefined' && error != null &&
                                ...
                                console.log("###master: register subid=" + dataObj.subid + "; worker="+ job_state[dataObj.subid].worker + "###\n\n");
                                worker.on('message', function(message){
                                        if(message.type=='get_buffer'){
                                            console.log("master.get_buffer::subid=" + message.subid + "; worker=" + job_state[message.subid].worker);
                                            job_state[message.subid].worker.send(
                                                {"type":"buffer_resp", 
            ...
            try {
                ...
                if(dataObj.command == 'data'){
                    ...
                    if(typeof dataObj.subid != 'undefined' &&
                        typeof job_state[dataObj.subid] != 'undefined')
                    {
                        job_state[dataObj.subid].worker.terminate();
                        delete job_state[dataObj.subid];
                        respObj.stat = 'ok';
...
if(process.argv.length==4){
    ...
    server.on('error', function (exception){
            ok=false;
            if(JSON.stringify(exception).indexOf("EADDRINUSE")>-1)
                console.error("EADDRINUSE");
        });
    setTimeout(declareOk, 1000);
}
else{
...
var blah = setInterval(function(){
        if(i<5){
            requestObj.subid=i;
            requestObj.proc_config.script.timeout = Math.floor(Math.random()*10)*100;
...
setTimeout(function(){
                var keys = Object.keys(job_state);
                console.log("keys=" + JSON.stringify(keys));
                for(k in keys){
                    if(typeof job_state[k] != 'undefined'){
                        //console.log(job_state[k]);
                        job_state[k].worker.terminate();
                    }
                }
            }, 100000);*/
